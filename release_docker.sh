...
# Check if the images were removed successfully
if [ $? -ne 0 ]; then
    echo -e "${RED}Error: Failed to remove local Docker images.${NC}"
    exit 1
fi

echo -e "${GREEN}Successfully removed local Docker images.${NC}"
exit 0