  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('https://waitlist.hlab.es/waitlist/users/', {
        ...
        body: JSON.stringify({
          waitlist_name: 'estavia_quick_signup',
          email: email,
        }),
      });
      ...
      >
        ...
        <div className="bg-page rounded-lg p-6 transition ease duration-300">
          <div className="mx-auto p-2 mb-4">
            <div className="text-xl font-semibold text-default mb-4 text-center">{titleText}</div>
            <div className="text-default text-center">{descriptionText}</div>
          </div>
